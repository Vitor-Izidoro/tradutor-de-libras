...
_N_MAO = 63       # features por mão
_N_AMBAS = 126    # features com as duas mãos
...
def _reshape(frame_mao):
    ...
    return np.array(frame_mao, dtype=np.float32).reshape(21, 3)
...
def _ruido(pts, intensidade=0.005):
    return pts + np.random.normal(0, intensidade, pts.shape).astype(np.float32)
...
def _aplicar(pts, t, ruido_frame=None, escala_fator=None, cos_a=None, sin_a=None):
    """
    ...
    if t == _ruido:
        return pts + (ruido_frame if ruido_frame is not None
                      else np.random.normal(0, 0.005, pts.shape).astype(np.float32))
    elif t == _escala:
        return pts * (escala_fator if escala_fator is not None
                      else np.random.uniform(0.85, 1.15))
    elif t == _rotacao_2d:
        if cos_a is None:
            ang = np.radians(np.random.uniform(-15.0, 15.0))
...
def _processar_frame(frame_arr, transformacoes,
                     ruido_a=None, ruido_b=None,
                     escala_fator=None, cos_a=None, sin_a=None):
    """
    ...
    """
    duas_maos = (len(frame_arr) == _N_AMBAS)

    if duas_maos:
        mao_d = _reshape(frame_arr[:_N_MAO])
        mao_e = _reshape(frame_arr[_N_MAO:])
        ...
        outras = [t for t in transformacoes if t != _espelhamento]
...
#criador -> vitor izidoro
        ...
        for t in transformacoes:
            pts = _aplicar(pts, t, ruido_a, escala_fator, cos_a, sin_a)
        pts = pts - pts[0]
        return _flatten(pts)
...
def augmentar_frame(frame, transformacoes):
    ...
    """
    return _processar_frame(np.array(frame, dtype=np.float32), transformacoes)
...
def augmentar_sequencia(sequencia, transformacoes):
    ...
    """
    n = len(sequencia)
    frame_len = len(sequencia[0]) if sequencia else _N_MAO

    escala_fator = np.random.uniform(0.85, 1.15)
    ang = np.radians(np.random.uniform(-15.0, 15.0))
    cos_a, sin_a = np.cos(ang), np.sin(ang)

    if frame_len == _N_AMBAS:
        ruido_d = np.random.normal(0, 0.005, (n, 21, 3)).astype(np.float32)
        ruido_e = np.random.normal(0, 0.005, (n, 21, 3)).astype(np.float32)
    else:
        ruido_u = np.random.normal(0, 0.005, (n, 21, 3)).astype(np.float32)

    resultado = []
    for fi, frame in enumerate(sequencia):
        arr = np.array(frame, dtype=np.float32)
        if frame_len == _N_AMBAS:
            r = _processar_frame(arr, transformacoes,
                                  ruido_a=ruido_d[fi], ruido_b=ruido_e[fi],
                                  escala_fator=escala_fator, cos_a=cos_a, sin_a=sin_a)
        else:
            r = _processar_frame(arr, transformacoes,
                                  ruido_a=ruido_u[fi],
                                  escala_fator=escala_fator, cos_a=cos_a, sin_a=sin_a)
...
def gerar_amostras_aumentadas(features, labels, mode="rf", n_aumentos=5, seed=42):
    ...
    for amostra, label in zip(features, labels):
        ...
        if mode == "rf":
            esp = _processar_frame(np.array(amostra, dtype=np.float32), [_espelhamento])
...
if __name__ == "__main__":
    ...
    l_rf = ["oi"] * 3 + ["tchau"] * 3
    f_aug, l_aug = gerar_amostras_aumentadas(f_rf, l_rf, mode="rf", n_aumentos=5)
    assert len(f_aug) == len(l_aug) and len(f_aug[0]) == 63
    print("[RF 1 mão] OK")

    print("\n=== Teste: 2 mãos (126 features) ===")
    f_rf2 = [np.random.randn(126).tolist() for _ in range(6)]
    f_aug2, l_aug2 = gerar_amostras_aumentadas(f_rf2, l_rf, mode="rf", n_aumentos=5)
    ...
    l_lstm = ["oi"] * 2 + ["tchau"] * 2
    f_aug3, l_aug3 = gerar_amostras_aumentadas(f_lstm, l_lstm, mode="lstm", n_aumentos=5)
    ...
    f_aug4, l_aug4 = gerar_amostras_aumentadas(f_lstm2, l_lstm, mode="lstm", n_aumentos=5)
